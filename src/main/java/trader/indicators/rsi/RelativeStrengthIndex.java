...
public final class RelativeStrengthIndex implements Indicator {
    ...
    RelativeStrengthIndex(long period, AppliedPrice appliedPrice, CandlesUpdater candlesUpdater){
        this.period = period;
    ...
    private void calculateRSI(List<Candlestick> candlestickList) {
        ...
        for (int i = 1; i <=this.period ; i++) {
            BigDecimal currentPrice = this.appliedPrice.apply(candlestickList.get(i).getMid());
            BigDecimal previousPrice =  this.appliedPrice.apply(candlestickList.get(i-1).getMid());
